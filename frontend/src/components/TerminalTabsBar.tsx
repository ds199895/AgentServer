...
import { ChevronDown, ChevronRight, Columns2, CornerDownLeft, LoaderCircle, Maximize2, MonitorPlay, Pin, Plus, Search, XIcon } from 'lucide-react'
...
type Props = {
  ...
  onPreviewTerminal: (id: string) => void
  onPinTerminal: (id: string) => void
  onRequestTerminate: (session: TerminalSession) => void
  onClone: (source: TerminalSession) => void
  onPreviewServices: (source: TerminalSession) => void
}
      ...
      <nav aria-label="终端导航" className="flex min-w-0 items-center border-b border-border bg-[#0b1016] px-2 py-[5px] max-md:px-1.5">
        ...
        >
          {terminalGroups.map((group) => {
            ...
            const importantSessions = [
              ...group.sessions.filter((session) => session.id === activeId),
              ...group.sessions.filter((session) => session.id !== activeId && placements.get(session.id)?.visible),
            ]
            const displayedSessions = importantSessions.slice(0, 8)
            for (const session of [...group.sessions].reverse()) {
              if (displayedSessions.length >= 8) break
            ...
            return (
              ...
              >
                <button
                  ...
                  className="flex max-w-[150px] min-w-[72px] cursor-pointer items-center gap-1.5 border-r border-[#26323c] px-2.5 text-left hover:bg-[#17212a] focus-visible:bg-[#17212a] max-md:max-w-[110px] max-md:min-w-[58px] max-md:px-2"
                >
                  {groupExpanded
                    ? <ChevronDown className="size-2.5 flex-none text-[#71808b]" />
                    : <ChevronRight className="size-2.5 flex-none text-[#71808b]" />}
                  <i
                    aria-hidden="true"
                  ...
                  />
                  <strong className={cn('min-w-0 flex-1 truncate text-[10px] font-semibold text-[#b9c5ce] max-md:text-[9px]', groupFocused && 'text-[#edf4f9]')}>
                    {groupName}
                  </strong>
                ...
                {groupExpanded && (
                  <div className="flex flex-none items-stretch">
                    <div role="list" aria-label={`${groupName} 终端`} className="flex flex-none items-stretch">
                      {displayedSessions.map((session) => {
                        ...
                        const terminalLabel = display?.label || session.name
                        const shortId = display?.shortId || session.id.slice(0, 8)
                        return (
                          <span
                            key={session.id}
                            role="listitem"
                          ...
                          >
                            <button
                              ...
                              type="button"
                              onClick={(event) => {
                                if (event.detail <= 1) onPreviewTerminal(session.id)
                              }}
                              onDoubleClick={(event) => {
                                event.preventDefault()
                                event.stopPropagation()
                                onPinTerminal(session.id)
                              }}
                              aria-current={focused ? 'page' : undefined}
                              aria-label={`预览 ${groupName} ${terminalLabel} ${shortId}，双击固定`}
                              title={`${groupName} · ${terminalLabel} · ${session.id}${placement?.pane ? ` · 窗格 ${placement.pane}` : ''} · 单击预览，双击固定`}
                              className={cn(
                                'flex max-w-[190px] min-w-0 touch-manipulation cursor-pointer items-center gap-1 bg-transparent pr-2 pl-[3px] text-[#72808b] hover:bg-[#17212a] hover:text-[#c2ced6] focus-visible:outline-1 focus-visible:outline-primary max-md:max-w-[150px] max-md:pr-1.5',
                                visible && 'text-[#9acbb3]',
                            ...
                            >
                              ...
                              <span className="min-w-0 truncate text-[9px] font-medium">{terminalLabel}</span>
                              <code className="flex-none font-mono text-[7px] text-[#63717c] max-md:hidden">{shortId}</code>
                              {placement?.pane && leaves.length > 1 && (
                                <span className={cn(
                                  'flex-none rounded bg-[#1b252d] px-1 font-mono text-[7px] text-[#75838e] max-md:hidden',
                            ...
                            <button
                              type="button"
                              aria-label={`固定 ${groupName} ${terminalLabel}`}
                              title="固定到终端窗格"
                              onClick={(event) => {
                                event.stopPropagation()
                                onPinTerminal(session.id)
                              }}
                              className="grid w-7 flex-none touch-manipulation cursor-pointer place-items-center bg-transparent text-[#596873] hover:bg-[#193025] hover:text-primary focus-visible:outline-1 focus-visible:outline-primary max-md:w-8"
                            >
                              <Pin className="size-2.5" />
                            </button>
                            <button
                              type="button"
                              aria-label={`终止 ${groupName} ${terminalLabel} 后台会话`}
                              title="终止后台会话"
                              onClick={(event) => {
                                event.stopPropagation()
                                onRequestTerminate(session)
                              }}
                              className="grid w-7 flex-none touch-manipulation cursor-pointer place-items-center bg-transparent text-[#586570] hover:bg-[#362027] hover:text-[#ff8d98] focus-visible:outline-1 focus-visible:outline-[#ff8d98] max-md:w-8"
                            >
                    ...
                    {group.deviceId && (
                      <button
                        ...
                        title="预览设备本地开发服务"
                        onClick={() => onPreviewServices(cloneSource)}
      ...
      >
        ...
        >
          <DialogHeader className="border-b border-[#293641] px-5 pt-5 pb-4 pr-12">
            ...
            <DialogDescription className="text-[10px]">搜索设备、终端、工作区、服务端口或运行状态；打开结果会先以预览标签显示。</DialogDescription>
          </DialogHeader>
          ...
          <div id="terminal-search-results" role="listbox" aria-label="终端搜索结果" className="min-h-0 overflow-y-auto p-2">
            ...
            {!searchResults.length && (
              <div className="px-4 py-12 text-center text-[11px] text-[#65747f]">没有匹配的终端</div>
            )}
          </div>
