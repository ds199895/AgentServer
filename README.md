...
- 桌面端支持左右/上下递归分屏、拖拽或键盘调整比例并持久化布局。每个 P1…Pn 都是默认空的独立 editor group，拥有自己的水平 Tab 栏和当前 Tab；Tab 可在同一窗格内拖动排序，也可拖到其他窗格，拆分和拖动都不会隐式创建或终止后台终端。
- 顶部设备/本机分组保留完整终端 Tab，是终端目录与生命周期入口：在当前 active 窗格中单击会临时预览、双击或 Pin 会固定；同一窗格的新预览会替换旧预览，已固定终端不会被替换。
- 窗格内关闭 Tab 只取消该终端在此窗格的显示，关闭最后一个 Tab 仍保留空窗格；只有设备组终端 Tab 的“终止”操作才会结束后台会话。移动端/聚焦模式只投影当前窗格，不修改桌面端归属和比例。
...
- 支持递归空窗格分屏和 VS Code 式预览/固定 Tab，并在热缓存范围内保持已有 xterm 连接、选区与滚动位置。
- 大量会话时仅挂载当前可见与最近使用的终端，其余会话由 tmux 持续托管，再次打开时通过快照恢复。
...
恢复原标签与滚动历史。只有在顶部设备组的终端 Tab 中显式选择“终止”时，才会删除数据库记录并执行
`tmux kill-session`。
...
契约会创建真实本地终端，实际构建默认空的 P1/P2/P3 分屏，并验证设备组终端的
单击预览、预览替换、双击固定、各窗格独立切换以及跨窗格拖动；同时检查窗格 Tab 关闭只解除显示、
设备组终止才删除后台会话，并验证空窗格、布局恢复、工作区
