...
- 桌面端支持左右/上下递归分屏、拖拽或键盘调整比例并持久化布局；终端选择统一在其归属窗格中显示，不会因入口不同暗中搬移。可临时聚焦当前终端、恢复分屏，或只关闭窗格而保留后台会话；移动端自动回退为单窗格而不破坏桌面布局。
- 终端导航按设备/本机分组，显示工作区、窗格编号、可见/焦点和运行状态；快速切换器可搜索设备名、hostname、终端 ID、路径、服务端口与状态。
...
契约会创建真实本地终端，实际左右分屏、按活动窗格切换终端 Tab、用键盘调整比例并在刷新后
恢复布局；随后验证工作区
