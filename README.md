...
- 桌面端支持左右/上下递归分屏、拖拽或键盘调整比例并持久化布局。每个 P1…Pn 都是独立 editor group：拥有自己的水平 Tab 栏、当前 Tab、新建、关闭和拆分操作，在一个窗格切换不会影响其他窗格。
- 顶部设备/本机分组仅是全局设备导航，不再混充终端 Tab；窗格内 Tab 显示设备、可读名称、短 ID、工作区与运行状态。全局搜索可按设备名、hostname、终端 ID、路径、服务端口与状态 reveal 会话。
- 关闭最后一个 Tab 会保留空窗格；显式关闭窗格才收缩分屏，并且只解除该组的 Tab 归属，不终止后台会话。移动端/聚焦模式只投影当前窗格，不修改桌面端的 Tab 归属和比例。
...
- 支持在同一设备或本地工作区中创建递归终端分屏，并在热缓存范围内保持已有 xterm 连接、选区与滚动位置。
...
契约会创建真实本地终端，实际构建 P1/P2/P3 分屏、在每个窗格内独立新建和切换
Tab、验证其他窗格不受影响、用键盘调整比例，并在刷新后恢复每个窗格的归属与选中状态；
随后验证空窗格、关闭窗格保留后台会话、全局 reveal、工作区
